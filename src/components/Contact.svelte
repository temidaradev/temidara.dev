    ...
    async function handleSubmit() {
        if (!name || !email || !message) {
            submitStatus = {
                success: false,
                message:
                    "Prithee, fill out all fields as required by royal decree.",
            };
            return;
        }
...
<section class="contact-section relative py-16">
    ...
    <div class="relative text-center mb-12">
        ...
        ></div>
        <p class="text-violet-200 mt-4 mb-2">
            Forge a Connection with My Self!
        </p>
        <p class="text-sm text-violet-300/70">
            Prithee, send forth a missive for potential alliances or merely to
            extend thine greetings
        </p>
    </div>
